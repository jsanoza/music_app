...
class Lyrics {
  ...
  Future<String> getLyrics({String? track, String? artist}) async {
    if (track == null || artist == null) throw Exception("track and artist must not be null");

    String lyrics;

    // try multiple queries
    try {
      lyrics = (await http.get(Uri.parse(Uri.encodeFull('${_url}$track by $artist lyrics')))).body;
      ...
      try {
        lyrics = (await http.get(Uri.parse(Uri.encodeFull('${_url}$track by $artist song lyrics')))).body;
        lyrics = lyrics.split(_delimiter1).last;
        lyrics = lyrics.split(_delimiter2).first;
        if (lyrics.indexOf('<meta charset="UTF-8">') > -1) throw Error();
      } catch (_) {
        try {
          lyrics = (await http.get(Uri.parse(Uri.encodeFull('${_url}${track.split("-").first} by $artist lyrics')))).body;
          lyrics = lyrics.split(_delimiter1).last;
          lyrics = lyrics.split(_delimiter2).first;
          if (lyrics.indexOf('<meta charset="UTF-8">') > -1) throw Error();
        } catch (_) {
